...
OUTDIR="/mnt/d/RNAseq_oct24/results_281024"
if [[ "$1" ]]; then
 if [[ "$1" == "-h" || "$1" == "--help" ]]; then
  usage
  exit 1
...
## load variables
if [[ ! -f ./rnaseq_pipeline.config241024.sh ]]; then
 usage
 echo "Error: configuration file (rnaseq_pipeline.config.sh) missing!"
 exit 1
fi

source ./rnaseq_pipeline.config241024.sh
WRKDIR=$(pwd -P)
errprog=""
if [[ ! -x $SAMTOOLS ]]; then
    errprog="samtools"
...
if [[ $OUTDIR != "/mnt/d/RNAseq_oct24/results_281024"
 ]]; then
  mkdir -p $OUTDIR
  cd $OUTDIR
fi
...
for ((i=0; i<=${#reads1[@]}-1; i++ )); do
    ...
    $STRINGTIE -e -p $NUMCPUS -G ${GTFFILE} -o ${ALIGNLOC}/${sample}.gtf \
...
$STRINGTIE --merge -e -p $NUMCPUS -G  ${GTFFILE} \
